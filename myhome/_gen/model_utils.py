...
class ModelComposed(OpenApiModel):
    ...
    def get(self, name, default=None):
        ...
        if len_values == 0:
            ...
            raise ApiValueError(
                "Values stored for property {} in {} differ when looking "
...
def check_validations(
    ...
    if (is_json_validation_enabled('exclusiveMaximum', configuration) and
        ...
        raise ApiValueError(
            "Invalid value for `{}`, must be a value less than `{}`".format(
                input_variable_path[0],
    ...
    if (is_json_validation_enabled('pattern', configuration) and
        ...
        err_msg = r"Invalid value for `{}`, must match regular expression `{}`".format(
            input_variable_path[0],
        ...
        if flags != 0:
            ...
            err_msg = fr"{err_msg} with flags=`{flags}`"
...
def type_error_message(var_value=None, var_name=None, valid_classes=None,
    ...
    msg = (
        "Invalid type for variable '{}'. Required {} type {} and "
        "passed type was {}".format(
            var_name,
            key_or_value,
...
def get_valid_classes_phrase(input_classes):
    ...
    if len(all_class_names) == 1:
        return f'is {all_class_names[0]}'
    return "is one of [{}]".format(", ".join(all_class_names))
...
def get_oneof_instance(cls, model_kwargs, constant_kwargs, model_arg=None):
    ...
    if len(oneof_instances) == 0:
        ...
        print(f"SCHEMAS: {oneof_instances!r}")
        print("SCHEMA TYPES: %r" % ([type(s) for s in oneof_instances]))
        raise ApiValueError(
